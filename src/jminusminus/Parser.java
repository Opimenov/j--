...
public class Parser {
	...
	/**
	 * return true iff there is a colon two tokens
	 */
	private boolean seeCOL() {
		scanner.recordPosition();
//		System.out.println("are we here<<<<<<<<<<<<<<<<<<<<<<<<<<<");
//		System.out.println(scanner.token().image());
		scanner.next();
		scanner.next();
		scanner.next();
		boolean res = see(COL);
	...
	private JClassDeclaration classDeclaration(ArrayList<String> mods) {
		...
		return new JClassDeclaration(line, mods, name, superClass, classBody(),null);
	}
	...
	private JMember memberDecl(ArrayList<String> mods) {
		...
		ArrayList<Type> thro_exc = new ArrayList<Type>(); 
		if (seeIdentLParen()) {
			// A constructor
			mustBe(IDENTIFIER);
			String name = scanner.previousToken().image();
			...
			if (have(VOID)) {
				...
				ArrayList<JFormalParameter> params = formalParameters();
				if (have(THROWS)) {
					thro_exc.add(qualifiedIdentifier());
					while (more) {
						if (have(COMMA)) {
							thro_exc.add(qualifiedIdentifier());
						}
	...
	private JStatement statement() {
		...
		if (see(LCURLY)) {
			...
			if (seeCOL()) {
				Type mType = type();
				mustBe(IDENTIFIER);
				String id = scanner.previousToken().image();
				mustBe(COL);
				JExpression expr = expression();
				mustBe(RPAREN);
				JStatement forStatem = statement();
				return new JForEnhancedStatement(line, mType, id, expr, forStatem); 
			} else {
				JForInit initFor = null;
				JExpression testConditionFor = null;
				JForUpdate updateFor = null;
				JStatement statementFor = null;
				if (!have(SEMI)) initFor = forInit();
				mustBe(SEMI); 
				if (!have(SEMI)) testConditionFor = expression();
				mustBe(SEMI);
				if (!have(SEMI)) updateFor = forUpdate();
				mustBe(RPAREN);
				statementFor = statement();
				return new JForRegularStatement(line, initFor, testConditionFor,
						updateFor, statementFor);
			}
		} else if (have(DO)) {
...
//				mustBe(RCURLY);
				...
				//if no CATCH then must be FINALLY
				mustBe(FINALLY) ;
				finallyBlock = block();
			}
			return new JTryStatement(line, tryBlock, param, catchBlock, finallyBlock);
		} else if (have(SWITCH)) {
			JSwitchBlockStatementGroup stGroup = null;
			/**
	...
	private JForInit forInit() {
		...
		if (seeLocalVariableDeclaration()) {
			if (have(FINAL)) {
				Type mType = type();
				ArrayList<JVariableDeclarator> vars = variableDeclarators(mType);
				return new JForInit(line, null, mType, vars);
			} else {
				Type mType = type();
				ArrayList<JVariableDeclarator> vars = variableDeclarators(mType);
				return new JForInit(line, null, mType, vars);
			...
			return  new JForInit(line, statements, null, null);
		}
	}
	...
	private JForUpdate forUpdate() {
		...
		return new JForUpdate(line, updates);
	}
	/**
	 ...
	 * 								[Q_MARK assignmentExpression COL 
	...
	private JExpression conditionalExpression() {
		...
		if (have(Q_MARK)) {
			JExpression assignThis = assignmentExpression();
			mustBe(COL);
			JExpression orThis = conditionalExpression();
			return new JTernaryOp(line, condition, assignThis, orThis);
		}
		return condition;
	}
	...
	/*******************************************************************************
                    ADDING BITWISE OPERATION PARSING BELOW
	 *******************************************************************************/
	/**
	 * Parse a bitwiseBinary expression.
	 * 
	 * <pre>
	 *   bitwiseBinaryExpression ::= BITAND bitwiseExpression	 
	 *                              | BITXOR bitwiseExpression
	 *        	   	            | BITOR bitwiseExpression
	 * </pre>
	 * 
	 * @return an AST for a bitwiseBinaryExpression.
	 */
	...
	private JExpression andExpression() {
		...
		while (more) {
			if (have(BITAND)) {
				lhs = new JBitwiseANDOp(line, lhs, andExpression());
			}
			else if (have(BITOR)) {
				lhs = new JBitwiseOROp(line, lhs, andExpression());
			}
			else if (have(BITXOR)) {
	...
	/*******************************************************************************
                    ADDING BITWISE OPERATION PARSING ABOVE
	 *******************************************************************************/
	 ...
	 * 
	 * <pre>
	 *   literal ::= INT_LITERAL | LONG_LIRERAL | CHAR_LITERAL | STRING_LITERAL
	 *             | TRUE        | FALSE        | NULL
	 * </pre>
	 * 
