...
class Scanner {
	...
	boolean already_have_Plus_Or_Minus = false;
		...
		//multiline comment
			...
			nextCh();
			if (ch == EOFCH) {
			    notAnEndMultiline = false;
			    reportScannerError("multiline comment is not closed");
			    return getNextToken();
			}
			//if the next ch is '/' exit outer while
			else if (ch == '/') {
			    notAnEndMultiline = false;
			    nextCh(); // get rid off '/'
			    return getNextToken();
        ...
        case '~':
	    nextCh();
	    return new TokenInfo(COMPLEM, line);
        case '%':
            nextCh();
	    if (ch == '=') {
		nextCh();
        ...
        case '^':
            ...
            if (ch == '=') {
                nextCh();
                return new TokenInfo(BITXOR_ASSIGN, line);
            } else {
	    ...
	    if (ch == '|') {
		nextCh();
		return new TokenInfo(L_OR, line);
	    }
	    else {
		...
		if (ch == '>') {
		    ...
		    if (ch == '=') {
			nextCh();
			return new TokenInfo(UNS_R_SHIFT_ASSIGN, line);
		    } else {
			return new TokenInfo(UNS_R_SHIFT, line);
		    }
		}
		else {
	...
	case '\\':
		...
		for (int i = 0; i < 4; i++) {
			...
			reportScannerError(ch
			 + " found by scanner where hexadeciaml digit was expected.");
			return getNextToken();
		    }
		    else {
			buffer.append(ch);
	    ...
	    buffer.append(ch);
	    nextCh();
	    // next char must be a digit, othersiwe return DOT
	    if (!isDigit(ch)) {
		return new TokenInfo(DOT, ".", line);
	    }
	    buffer.append(ch);
	    nextCh();
	    while (belongsToFloatOrDubleButNotHex(ch)) { 
		// case of e or E		    
		...
		buffer.append(ch);
		nextCh();
		return new TokenInfo(LONG_LITERAL, buffer.toString(), line); // add this to the Token INFO
	    }
	    // 0f or 0F case
	    else if (ch == 'f' || ch == 'F') {
		buffer.append(ch);
	    ...
	    if (alredy_have_dot || already_have_E_or_e) return new TokenInfo(DOUBLE_LITERAL, buffer.toString(), line);
    ...
    private boolean belongsToFloatOrDubleButNotHex(char c) {
        return (isDigit(c) || c == 'f' || c == 'F' || c == 'd' || c == 'D' ||
		c == 'e' || c == 'E' || c == 'p' || c == 'P' || c == '+' || c == '-');
    }
